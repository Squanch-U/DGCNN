...
class AdaptiveAvgPool3D(nn.Cell):
    ...
    :math:`(N, C, D_{out}, H_{out}, W_{out})`. where :math:`N` is batch size. :math:`C` is channel number.
            ...
            for j in range(self.out_h):
                for k in range(self.out_w):
                    ...
                    out.append(self.mean_op(self.stride_slice_op(x,
                                                                 ...
                                                                 (1, 1, 1, 1, 1)), (2, 3, 4)))
        out = self.reshape(self.concat_op(out),
                           (n, c, self.out_d, self.out_h, self.out_w))
        return out
